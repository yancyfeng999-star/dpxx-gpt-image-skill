...
> "需要 RootFlowAI API Key 才能调用 gpt-image-2，请把你的 API Key 贴这里。我用一次后立即从内存释放，不写入任何文件。"

**用户**：`<ROOTFLOWAI_API_KEY>`

**Agent** 调 `ask_user(mode=form)` 先问 Q1：

> 卡片 1：选哪个分辨率？（1K / 2K / 4K，显示费用）

**Agent** 根据分辨率决定模型和比例范围，再调 `ask_user(mode=form)` 问 Q2+Q3+Q4：

> 卡片 2：选哪个尺寸比例？（1K/2K 展示 13 种；4K 只展示 6 种宽幅）
> 卡片 3：选哪类提示词模板？（T05 Logo / T01 写实 / T04 海报 / T13 UI / 其他）
> 卡片 4：主题是？（让我帮你想 / 我下一条发主题）

`quality` 默认 `high`，不要单独询问。只有用户明确要求"低质量预览 / low quality / 最低质量"时才传 `--quality low`；只有用户明确指定 medium 时才传 `--quality medium`。

**用户**：1K · 1:1 · T05 Logo · "主题：一个叫 Accio Work 的 AI 协作平台，主色蓝橙"
...
## 通用 checklist（每次跑前对一遍）

- [ ] `env | grep ROOTFLOWAI` 看 key 是否就绪；没有就 `ask_user(chat)` 拿一次性 key，**优先用 `--api-key` 直传**（跨 shell 兼容）。
- [ ] Q1 分辨率 / Q2 比例 已在 `ask_user(form)` 里问到答案。
- [ ] Q1 = 4K → Q2 必须 ∈ `{16:9, 9:16, 2:1, 1:2, 21:9, 9:21}`，否则二次确认。
- [ ] `quality` 默认 `high`；只有用户明确要求低质量预览才改 `low`，只有用户明确指定 medium 才改 `medium`。
- [ ] Q3 模板 ID 选定 → `read references/prompt-patterns.md` **取出对应 T 段原文**再填。
- [ ] Q4 主题已确认（用户给的 / 你给的并征得同意）。
- [ ] `--profile` 和 `--model` 匹配密钥 lane；count / metered 都会回退读 `ROOTFLOWAI_API_KEY`。
- [ ] **CLI 是 `--output-dir`，不是 `--out`**；`--prefix` 显式给，方便定位文件。
- [ ] 跑完用 `see_image` 看一眼成品再汇报。
- [ ] 最终消息里 markdown 嵌图 + 一句话技术摘要 + 一句话迭代提示。

## 通用 checklist（每次跑后对一遍）
