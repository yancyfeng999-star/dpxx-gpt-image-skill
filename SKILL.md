...
> **CLI 参数速查**（已实跑验证）：必传 `--prompt`；可选 `--api-key` `--profile {auto,metered,count}` `--model` `--size` `--quality` `--n` `--image`（图生图，可重复 ≤16 次）`--output-dir` `--prefix` `--response-path` `--timeout`。**注意：不是 `--out`，是 `--output-dir`。**
...
python3 {SKILL_DIR}/scripts/generate_image.py \
  ...
  --prompt "..." --size 2:3 --output-dir ./out --prefix img
```
...
python3 {SKILL_DIR}/scripts/generate_image.py \
  ...
  --prompt "..." --size 21:9 --output-dir ./out --prefix img \
...
>   --prompt "$PROMPT" --size 9:16 --output-dir "$OUT_DIR" --prefix img \
...
python3 {SKILL_DIR}/scripts/generate_image.py \
  ...
  --size 1:1 --output-dir ./out --prefix figure
```
...
python3 {SKILL_DIR}/scripts/edit_image.py \
  ...
  --size 3:2 --output-dir ./out --prefix edit
```
...
python3 {SKILL_DIR}/scripts/generate_image.py \
  --api-key "<ROOTFLOWAI_API_KEY>" \
  --prompt "..." --size 1:1 --output-dir ./out --prefix img
...
python3 {SKILL_DIR}/scripts/edit_image.py \
  ...
  --image <上一张落盘路径> \
  --prompt "<用户描述的改动>" \
  --size <原比例> --output-dir <原输出目录> --prefix edit
```
